...
def make_candlestick_chart(df: pd.DataFrame, symbol: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(x=df.index, y=sma20, name="SMA20",
                   line=dict(color="#ff9800", width=1.5, dash="dot")),
        row=1, col=1,
    )
    fig.add_trace(
        go.Scatter(x=df.index, y=sma50, name="SMA50",
...
def make_equity_curve(equity_curve: List) -> go.Figure:
    ...
    baseline = values[0]
    colors = ["#00e676" if v >= baseline else "#ff1744" for v in values]
    ...
    fig.add_hline(y=baseline, line_dash="dash", line_color="#aaaaaa",
...
def make_score_breakdown_chart(results_df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        y=top["symbol"], x=top["tech_score"] * 0.4,
        name="Technical (40%)", orientation="h",
        marker_color="#2196f3"
    ))
    fig.add_trace(go.Bar(
        y=top["symbol"], x=top["fund_score"] * 0.4,
        name="Fundamental (40%)", orientation="h",
        marker_color="#4caf50"
    ))
    fig.add_trace(go.Bar(
        y=top["symbol"], x=top["market_score"] * 0.2,
        name="Market (20%)", orientation="h",
        marker_color="#ff9800"
    ))
    fig.update_layout(
        barmode="stack",
...
def render_performance_tab():
    ...
    if len(trader.closed_trades) >= 5:
        ...
        fig_corr = px.scatter(
            ...
            color_discrete_map={"Win": "#69f0ae", "Loss": "#ff5252"},
            template="plotly_dark",
        )
...
def main():
    ...
    # Custom CSS for professional dark terminal aesthetic
    st.markdown("""
    <style>
    ...
    .stMetricValue { color: #58a6ff !important; font-family: 'JetBrains Mono', monospace; font-size: 1.4rem !important; }
    .stMetricDelta { font-family: 'JetBrains Mono', monospace; }
    ...
    .stButton > button[kind="primary"]:hover {
        box-shadow: 0 0 24px rgba(56,139,253,0.5);
    ...
    .disclaimer-box {
        background: #1c2128; border: 1px solid #da3633;
        border-radius: 8px; padding: 12px 16px;
        font-size: 0.78rem; color: #f85149;
        margin-bottom: 12px;
    }
    .stCode { font-family: 'JetBrains Mono', monospace; font-size: 0.82rem; }
    </style>
    ...
    <div style="display:flex; align-items:center; gap:16px; margin-bottom:4px;">
        <div style="font-size:2.4rem; font-family:'JetBrains Mono'; font-weight:600;
    ...
    <div class="disclaimer-box">
        ...
        Stock markets involve substantial risk including loss of principal.
